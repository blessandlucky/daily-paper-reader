...
score: 7.0
evidence: 电化学反应与固液界面的原子级建模
tldr: 本文探讨了电化学反应的原子尺度建模在可持续能源技术中的核心作用。针对目前在操作条件下模拟带电固液界面所面临的预测性与可靠性挑战，欧洲COST行动EU-CONCERT项目提出了通过系统基准测试和交叉验证来推进建模技术。文章总结了当前技术现状，明确了优先解决的方法论难题，并为电化学建模社区提供了未来发展的路线图。
source: arxiv
selection_source: fresh_fetch
motivation: 现有的电化学固液界面原子尺度建模在操作条件下缺乏足够的预测能力和定量可靠性。
method: 通过EU-CONCERT行动，利用系统基准测试和跨方法验证来评估并改进现有的计算模型。
result: 明确了当前电化学建模的技术现状，识别了关键的方法论挑战，并定义了急需的基准测试标准。
conclusion: 本文为电化学反应原子尺度建模领域提供了重要的指导方针和未来发展的战略路线图。
---

## 摘要
计算建模在深化我们对电化学反应的理解方面发挥着核心作用，并因此可以指导可持续能源技术的理性开发。尽管在方法论上取得了显著进展，但在工况（operando）条件下实现对带电固液界面的预测性且定量可靠的模拟仍然是一项重大挑战。最近，欧洲 COST 行动 EU-CONCERT（高效可再生技术电催化欧洲协作网络）启动，旨在通过系统性的基准测试和交叉验证来推进电化学反应的原子尺度建模。本立场文件概述了该联盟对当前技术现状的看法，确定了亟待解决的优先方法论挑战，并定义了迫切需要的基准测试。本文旨在为该领域提供指南，并作为 EU-CONCERT COST 行动的路线图。
