...
tldr: 针对微流控设计长期依赖经验原型且缺乏预测能力的现状，本研究开发了一种高保真三维流固耦合（FSI）框架。该框架结合高阶ALE有限元法与局部层次动态网格策略，能精确模拟复杂多尺度流体动力学。通过对确定性侧向位移结构的验证，该框架实现了亚微米级的轨迹预测，揭示了粒子垂直迁移等深层物理机制，为微流控数字化设计提供了强有力的预测工具。
source: biorxiv
selection_source: fresh_fetch
motivation: 传统的二维或解耦模拟无法捕捉微流控中真实的流体与粒子耦合行为，限制了预测性数字化设计的发展。
method: 建立了一种结合高阶任意拉格朗日-欧拉映射有限元法与局部层次动态网格策略的三维流固耦合框架。
result: 该框架在DLD结构中实现了亚微米级的粒子轨迹预测，并揭示了局部尺寸阈值的M型波动及粒子的动态垂直迁移现象。
conclusion: 该高保真框架为复杂微流控结构的数字化评估与优化提供了基础设施，推动了从经验试错向精准仿真驱动工程的转变。
---

## 摘要
微流控技术的商业化成熟仍受限于经验性的原型开发以及预测性数字设计能力的缺失。由于优化被动粒子分离等先进技术从根本上取决于流体动力学与粒子力学的精确耦合，传统的二维或解耦流体模拟本质上无法捕捉真实的多尺度行为。为了弥补这一差距，我们建立了一个高保真三维流固耦合框架，该框架结合了基于高阶任意拉格朗日-欧拉（ALE）映射的有限元方法与局部层次化动态网格策略。该架构旨在精确解析复杂的多尺度流体动力学，并利用确定性侧向位移（DLD）结构作为严格的测试案例。通过对硬质微球和肿瘤细胞的实验数据验证，该框架能以亚微米级的精度预测输运轨迹和临界分离直径。至关重要的是，该模拟明确解析了局部尺寸阈值的M型空间波动以及粒子的动态垂直迁移。揭示这些隐藏的物理机制为混合模式输运等备受争议的现象提供了确定性的解释。通过实现对复杂非周期性结构的严谨计算机模拟评估，该框架成为预测性结构优化的强大工具。这些能力为微流控数字设计奠定了必要的基础设施，加速了从经验性试错向精密模拟驱动工程的转变。
