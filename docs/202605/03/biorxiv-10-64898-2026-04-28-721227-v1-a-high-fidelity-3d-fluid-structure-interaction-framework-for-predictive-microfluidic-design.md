...
tldr: 针对微流控设计过度依赖经验原型且缺乏预测能力的现状，本研究开发了一种高保真三维流固耦合（FSI）框架。该框架结合高阶ALE有限元法与局部层次动态网格策略，能精确模拟复杂多尺度流体动力学。通过在确定性侧向位移结构中的验证，该框架实现了亚微米级的轨迹预测，并揭示了粒子垂直迁移等深层物理机制，为微流控数字化设计提供了强有力的预测与优化工具。
source: biorxiv
selection_source: fresh_fetch
motivation: 传统的二维或解耦模拟无法捕捉微流控中真实的流体与粒子耦合行为，限制了预测性数字化设计的发展。
method: 建立了一种结合高阶任意拉格朗日-欧拉（ALE）映射有限元法与局部层次动态网格策略的三维流固耦合框架。
result: 该框架在DLD结构中实现了亚微米级的粒子轨迹预测，并揭示了局部尺寸阈值的M型波动及粒子的动态垂直迁移现象。
conclusion: 该高保真框架为复杂微流控结构的数字化评估与优化提供了基础，推动了从经验试错向精准仿真驱动工程的转变。
---
