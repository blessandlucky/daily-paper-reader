...
score: 6.5
evidence: 包括锂金属在内的锂电池负极化学体系中的SEI生长动力学
...
## 摘要
固体电解质界面（SEI）的增长通常针对特定电池和特定化学体系进行建模，但其底层的动力学标度规律很少在不同化学体系间进行验证。通过汇集涵盖石墨、硅复合材料、锂金属和无负极四种负极配置的公开长循环数据集中的循环解析数据，我们表明在其中三种化学体系中，累积界面损失指数 Lambda_int 遵循抛物线定律 Lambda_int = A_chem * sqrt(1 - Theta_Li)，其指数在实验误差范围内与 alpha = 1/2 无异。虽然特定化学体系的前因子 A_chem 跨越了一个数量级，但扩散限制的抛物线动力学得以保留。第四种化学体系，即无负极配置，偏离了该规律，其超抛物线指数 alpha 约为 0.77，这与成核控制的增长机制一致。我们利用适用于界面形成的 Tammann-Deal-Grove 抛物线增长框架对结果进行了合理解释，并确定了恢复普适性的条件。观察到的规律性将 SEI 建模的复杂性降低到每个化学体系仅需一个速率常数，并为下一代电池形态提供了严谨的可证伪性测试。
